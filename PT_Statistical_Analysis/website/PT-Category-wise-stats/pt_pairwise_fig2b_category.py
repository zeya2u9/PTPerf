...
base_dir = input("Enter absolute path to selenium-based website access time between category pairs of PTs")
data = {}
os.chdir(base_dir)
...
def ci_func(g1,g2):
    ...
    values2 = st.norm.interval(alpha=0.95, loc=np.mean( diff1 ), scale=st.sem(diff1) )
    return values2
...
data.to_csv(f"{path}/fig_2b_categorywise.csv", index=False)

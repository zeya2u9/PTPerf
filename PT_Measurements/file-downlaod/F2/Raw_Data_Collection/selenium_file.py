...
pathc_5 = f"cd {os.getenv('HOME')}/Cloak/ && build/ck-client -c ckclient.json -s  &"
...
def testWebsite(website, iteration, pt_num, browser):
	...
	try:
		end = int( (run(f"stat -c '%Y' /root/Downloads/{website[23:]}".split(), capture_output=True).stdout).decode()[1:-2]  ) 
		time_taken = end -start
